...
export default function LoginPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = "error" in data ? data.error : "Login failed";
        throw new Error(msg);
      }
    ...
    <main className="mx-auto max-w-md px-6 py-12">
      ...
      <form onSubmit={onSubmit} className="mt-6 space-y-4">
        ...
        <div className="flex items-center justify-between pt-2">
          <a className="text-sm text-gray-300 underline" href="/register">
            Create account
          </a>
          <a className="text-sm text-gray-300 underline" href="/reset-password">
            Forgot password?
          </a>
        </div>
      </form>
